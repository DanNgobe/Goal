...
local Config = {
	...
	FormationWeight = {
		...
		Formation = 0.85,  -- 85% formation, 15% ball awareness
		Defensive = 0.9  -- 90% formation when defending
	},
...
function HandleBallPossession(slot, npc, humanoid, root, teamName)
	...
	if TryPass(slot, npc, root, teamName) then
		return
	end
...
function TryShoot(slot, npc, root, teamName)
	...
	local kickType = dist > 30 and "Air" or "Ground"
		...
		if slot.NPC ~= npc and slot.NPC.Parent then
			...
			if teammateRoot and teammateHumanoid and teammateHumanoid.Health > 0 then
				...
				if dist > 5 and dist < Config.Distance.PassRange then
					...
					local aheadAmount = myDistToGoal - teammateDistToGoal
					local score = aheadAmount - (dist * 0.2)
